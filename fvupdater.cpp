...
void FvUpdater::UpdateInstallationConfirmed()
{
    ...
    {
        showErrorDialog(QStringLiteral("请手动打开连接"), true);
        return;
    }
...
bool FvUpdater::CheckForUpdates(bool silentAsMuchAsItCouldGet)
{
    ...
    if (QApplication::applicationVersion().isEmpty())
    {
        ...
        QString appVersion = FV_APP_VERSION;
...
void FvUpdater::httpFeedDownloadFinished()
{
    ...
    if (m_reply->error())
    {
        ...
        // Error.
        showErrorDialog(QStringLiteral("更新下载失败: %1.").arg(m_reply->errorString()), false);
...
bool FvUpdater::xmlParseFeed()
{
    ...
    while (! m_xml.atEnd())
    {
        ...
        else if (m_xml.isCharacters() && ! m_xml.isWhitespace())
        {
            ...
            else if (currentQualifiedTag == "fervor:releaseNotesLink")
            {
        ...
        if (m_xml.error() && m_xml.error() != QXmlStreamReader::PrematureEndOfDocumentError)
        {
            ...
            showErrorDialog(QStringLiteral("升级信息解析失败: %1 %2。")
                            .arg(QString::number(m_xml.lineNumber()), m_xml.errorString()), false);
            return false;
    ...
    showInformationDialog(QStringLiteral("当前是最新版本。"), false);
        ...
        QString xmlEnclosureType)
{
    ...
    if (xmlReleaseNotesLink.isEmpty())
    {
        if (xmlLink.isEmpty())
        {
            showErrorDialog(QStringLiteral("升级失败: \"release notes\" link is empty"), false);
            return false;
        }
        else
        {
    ...
    {
        showErrorDialog(QStringLiteral("升级失败: 无效的 \"release notes\" 连接"), false);
        return false;
    }
    if (xmlEnclosureUrl.isEmpty() || xmlEnclosureVersion.isEmpty() || xmlEnclosurePlatform.isEmpty())
    {
        showErrorDialog(QStringLiteral("升级失败: 无效的 \"enclosure\" 下载连接"), false);
        return false;
    }
    ...
    if (FVIgnoredVersions::VersionIsIgnored(xmlEnclosureVersion))
    {
        ...
        showInformationDialog(QStringLiteral("当前没有更新"), false);
...
void FvUpdater::showErrorDialog(QString message, bool showEvenInSilentMode)
{
    ...
    dlFailedMsgBox.setText(QStringLiteral("错误"));
...
void FvUpdater::showInformationDialog(QString message, bool showEvenInSilentMode)
{
    ...
    dlInformationMsgBox.setText(QStringLiteral("信息"));
